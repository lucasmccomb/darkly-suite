# Chrome Web Store Listing — Darkly Suite

## Extension Name

Darkly Suite — Dark Mode for Google Workspace

## Short Description (132 chars max)

All-in-one dark mode for Gmail, Sheets, and Docs. One extension, one subscription — theme presets, scheduling, and OS sync included.

## Detailed Description

Darkly Suite brings a carefully engineered dark mode to Gmail, Google Sheets, and Google Docs — all from a single extension with a single subscription. Stop juggling three separate dark mode extensions. Darkly Suite gives you unified dark mode across Google Workspace with per-app settings, shared theme presets, and one license that covers everything.

WHY DARKLY SUITE?

Most dark mode extensions only work on one Google app. If you use Gmail, Sheets, and Docs daily, that means installing three extensions, managing three subscriptions, and dealing with three separate settings panels. Darkly Suite replaces all of that with one extension that's purpose-built for each app.

- One extension, three apps — Gmail, Sheets, and Docs dark mode in a single install
- One subscription — covers all three apps at a lower price than buying individually
- Per-app settings — configure dark mode independently for each app
- Shared preferences — theme presets and scheduling sync across all three apps
- Conflict detection — works gracefully alongside standalone Darkly extensions if you have them installed

GMAIL FEATURES

- Instant dark mode — Toggle dark mode on and off from a button inside Gmail
- Gmail-specific overrides — Carefully tuned for Gmail's interface including compose windows, email threads, popovers, and settings pages
- Settings panel inside Gmail — Access all settings from a toolbar button or sidebar icon without leaving your inbox
...
- Monthly: $2.99/mo (vs $2.97/mo for three separate extensions)
...
The lifetime plan is the best value: pay once and get dark mode for Gmail, Sheets, and Docs forever. All plans include full access to every feature across all three apps.

HOW IT WORKS

Install Darkly Suite and dark mode is ready across Gmail, Google Sheets, and Google Docs. Each app gets its own toggle button in the toolbar for instant on/off control.
...
WORKS WITH STANDALONE DARKLY EXTENSIONS

Already using Darkly for Gmail, Darkly for Sheets, or Darkly for Docs? Darkly Suite detects installed standalone extensions and prevents double-injection. The first extension to load on a page claims it, and the other gracefully steps aside. You can transition to the Suite at your own pace.

PRIVACY

Darkly Suite does not read your emails, spreadsheet data, or document content. It does not collect personal information or send analytics. Your preferences are stored locally using Chrome's storage API. See our full privacy policy at https://darklysuite.com/privacy.

SUPPORT

Visit https://darklysuite.com for documentation and support, or email admin@darklysuite.com.
